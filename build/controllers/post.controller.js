...
exports.deletePost = exports.updatePost = exports.createPost = exports.deleteImage = exports.getMyPosts = exports.getPost = exports.getUserPost = exports.getAllPosts = void 0;
...
exports.getAllPosts = (0, express_async_handler_1.default)(async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = 10;
    const skip = (page - 1) * limit;
    const viewer_id = req.user?._id;
    // Step 1: Get all private user IDs
    const privateUserIds = await user_model_1.default.find({ account_type: 'private' }).distinct('_id');
...
exports.getUserPost = (0, express_async_handler_1.default)(async (req, res) => {
    const { user_id } = req.params;
    const user = await user_model_1.default.find({ _id: user_id, account_type: "public", verified: true });
    const viewer_id = req.user._id;
    if (!user)
    ...
    const posts = await post_model_1.default.find({ user_id, type: "posted" })
        .sort({ createdAt: -1 })
        .populate([
        { path: "images", select: "image_url _id" },
...
exports.getPost = (0, express_async_handler_1.default)(async (req, res) => {
    const { post_id } = req.params;
    // Get all private user IDs
    const privateUserIds = await user_model_1.default.find({ account_type: 'private' }).distinct('_id');
    const post = await post_model_1.default
        .findOne({
        _id: post_id,
        type: "posted",
        post_type: "post",
...
exports.getMyPosts = (0, express_async_handler_1.default)(async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = 6;
    const skip = (page - 1) * limit;
    const user_id = req.user._id;
    // Step 1: Fetch paginated posts
    const posts = await post_model_1.default.find({ user_id })
        .sort({ createdAt: -1 })
        .populate([
        { path: "images", select: "image_url _id" },
        { path: "user_id", select: "name username _id account_type profile_pic" }
...
exports.deleteImage = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const existingImage = await image_model_1.default.findOne({ _id: image_id, user_id });
    if (!existingImage) {
        throw new error_helper_1.AppError("Image Not Found", 404);
    }
    // Fetch the associated post
    const post = await post_model_1.default.findById(existingImage.post_id);
    if (!post) {
    ...
    // Delete the image
    await image_model_1.default.findByIdAndDelete(image_id);
...
exports.createPost = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const uploadedUrls = await Promise.all(req.files.map((file) => (0, cloud_helper_1.default)(file.buffer)));
    // Temporarily store image URLs, will replace with ObjectIds
    const imageDocs = await image_model_1.default.insertMany(uploadedUrls.map(url => ({
        image_url: url,
        user_id
    })));
    ...
    await Promise.all(imageDocs.map((img) => image_model_1.default.findByIdAndUpdate(img._id, { post_id: post._id })));
    res.json({ status: true, message: "Post Created", data: post });
});
exports.updatePost = (0, express_async_handler_1.default)(async (req, res) => {
    const { post_id } = req.params;
    const { description, hashtags, type, post_type } = req.body;
    const user_id = req.user._id;
    const existingPost = await post_model_1.default.findOne({ _id: post_id, user_id });
    if (!existingPost)
        throw new error_helper_1.AppError("Post Not Found", 404);
    ...
    const updatedPost = await post_model_1.default.findOneAndUpdate({ _id: post_id, user_id }, {
        description,
        hashtags: JSON.parse(hashtags),
        type,
        post_type,
...
exports.deletePost = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!existingPost)
        throw new error_helper_1.AppError("Post Not Found", 404);
    await post_model_1.default.findByIdAndDelete(post_id);
    await image_model_1.default.deleteMany({ post_id, user_id });
