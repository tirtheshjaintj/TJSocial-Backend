...
exports.bookmarkPost = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const existingLike = await bookmark_model_1.default.findOne({ post_id, user_id });
    if (existingLike) {
        await bookmark_model_1.default.deleteOne({ _id: existingLike._id });
        res.json({ status: true, message: "Removed From Bookmarks", data: false });
    }
    else {
        await bookmark_model_1.default.create({ post_id, user_id });
